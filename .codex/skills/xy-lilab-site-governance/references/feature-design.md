...
- Abstract source priority is PubMed, then PMC for PubMed-linked open full text, OpenAlex by DOI, and finally a batched Semantic Scholar DOI lookup. PubMed structured abstracts must join all labelled `AbstractText` sections. Secondary-index fallback is not used for letters, editorials, corrections, or case reports because indexed text may contain body text or data-availability boilerplate for those formats. A missing upstream abstract remains missing rather than being inferred or generated.
