...
- The two tracks are presented as two clearly-headed blocks, in order: 招聘 first, then 招生. Each block opens with the shared `.v2-section__intro` accent-underlined `<h2>` header (the same accent-header component used on Clinical / Publications, but here used standalone — without the surrounding `.v2-section` wrapper — because the `<hr>` between tracks already supplies the inter-block spacing). The headers stand alone with no descriptive sub-line. The blocks are separated by that `<hr>`.
- The page carries `page_class: join`, which keys a small scoped heading-hierarchy block in `v2-pages.css`: the track `<h2>` is enlarged so it dominates, and the in-track sub-section `<h3>`s ("Eligibility and platform", "How to apply") get a hairline bottom rule so the tiers (page title › red-underlined track header › hairline-ruled sub-section › serif role title) read clearly. Keep this hierarchy scoped to the Join page; do not push these sizes onto the shared interior-page heading rules. Within the staff block, "Eligibility and platform" and "How to apply" remain `<h3>` subsections. Rely on this accent-header + hairline rhythm for separation; do not introduce heavy dividers, boxes, or cards (see design-philosophy: hierarchy from scale/spacing/contrast, not containers).
