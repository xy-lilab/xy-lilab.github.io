...
- The two tracks are presented as two clearly-headed blocks, in order: 招聘 first, then 招生. Each block opens with the shared `.v2-section__intro` accent-underlined `<h2>` header (the same accent-header component used on Clinical / Publications, but here used standalone — without the surrounding `.v2-section` wrapper — because the `<hr>` between tracks already supplies the inter-block spacing). The blocks are separated by that `<hr>`. Within the staff block, "Eligibility and platform" and "How to apply" remain `<h3>` subsections. Rely on this accent-header + hairline rhythm for separation; do not introduce heavy dividers, boxes, or cards (see design-philosophy: hierarchy from scale/spacing/contrast, not containers).
